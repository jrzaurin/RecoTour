...
def coupon_features(inp_dir, out_dir):
	...
	# Coupon features
	df_coupons_train = pd.read_pickle(os.path.join(inp_dir, 'ftrain', 'df_coupons_train.p'))
	df_coupons_train['set_type'] = 1
	...
	df_coupons_train = (df_coupons[df_coupons.set_type == 1]
	...
	# save files
	df_coupons_train.to_pickle(os.path.join(out_dir,"ftrain","df_coupons_train_feat.p"))
	pickle.dump(dict_of_mappings, open(os.path.join(out_dir, "ftrain", 'dict_of_mappings.p'), 'wb') )
