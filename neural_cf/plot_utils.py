...
def build_model_df(df):
    df_cp = df.copy()
    df_cp = df_cp[
        df_cp.modelname.str.contains("MLP|GMF") & ~df_cp.modelname.str.contains("MSE")
    ]
    df_cp["model"] = df_cp.modelname.apply(lambda x: "GMF" if "GMF" in x else "MLP")
    df_cp = df_cp.sort_values("best_hr", ascending=False).reset_index(drop=True)
    n_emb = [
        int(mn.split("n_emb_")[1].split("_")[0]) for mn in df_cp.modelname.tolist()
    ]
    df_cp["n_emb"] = n_emb
    df_cp = df_cp.sort_values(
        by=["model", "n_emb", "best_hr"], ascending=[True, True, False]
